...
class MetricComparer:
    def __init__(self, bigger_is_better: bool, can_be_equal: bool, metric_name: str, threshold: float = 0.0) -> None:
        """
        bigger_is_better is whether having a higher value is better or not True = yes False = no
        can_be_equal means we're ok with the previous value being the same as the new value e.g.
        we're ok with model_size being equal
        metric_name is the name of a metric we log to MLFlow
        threshold is how much better we want the new value to be compared to the old value
        """
    ...
    def is_metric_better(self, run: Run, best_run_data: dict[str, Any]) -> bool:
        if not best_run_data: #if we don't have previous models
            return True
...
class ModelSelector:
    ...
    def is_selected(self) -> bool:
        """
        True means we want to log this model to MLFlow
        """
