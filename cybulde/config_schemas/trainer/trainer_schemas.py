...
from cybulde.config_schemas.trainer import logger_schemas, callbacks_schemas
...
class TrainerConfig(LoggableParamsMixin):
    ...
    devices: str = "auto"
    num_nodes: int = 1  # SI("${infrastructure.vm_config.node_count}")
...
class GPUDev(TrainerConfig):
    ...
    limit_train_batches: float = .001
    limit_val_batches: float = .01
    limit_test_batches: float = .01
    logger: Optional[list[logger_schemas.LoggerConfig]] = field(default_factory=lambda: [logger_schemas.MLFlowLoggerConfig()])  # type: ignore
    callbacks: Optional[list[callbacks_schemas.CallbackConfig]] = field(default_factory=lambda: [
        ...
        callbacks_schemas.LearningRateMonitorConfig()])  # type: ignore
