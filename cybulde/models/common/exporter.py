...
class TarModelExporter:
    def __init__(
        self,
        model_state_dict_path: str, #reading saved parameters from disk
        model_config: Any,
    ...
    def _replace_module_in_model_config(self, config: Any) -> Any:
        for key, value in config.items():
            if isinstance(value, (dict, DictConfig)):
                self._replace_module_in_model_config(value) #recursive call to replace all paths

            if key == "_target_": #the location of the model changed to where we exported to
