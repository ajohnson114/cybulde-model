...
class FCLayer(Adapter):
    def __init__(self, 
        ...
        order: str= "LABDN" #Order of ops = Layer, Activation, Batch_norm, dropout, normalization
        ) -> None:
        super().__init__()
...
class MLPLayer(Adapter):
    def __init__(
        ...
        order: str = "LABDN",
        standardize_input: bool = True #should you do layer norms
    ) -> None:
        super().__init__()
        ...
        dropout_drop_probs = [0.0] * nrof_layers if dropout_drop_probs is None else dropout_drop_probs
        batch_norms = [False] * nrof_layers if batch_norms is None else batch_norms

        
        self.adapter = nn.Sequential()

        if standardize_input:
            self.adapter.add_module("standardize_input", nn.LayerNorm(output_feature_sizes[0], elementwise_affine=False))
